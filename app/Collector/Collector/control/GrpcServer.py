...
class GrpcServer:
    ...
    def initial(self, config_dict: dict[str, Union[str, dict]]) -> None:
        if config_dict is None:
            return
        self.__rpc_listen_address = config_dict.get('external_trigger_rpc_address', None)

    def update(self, config_dict: dict[str, Union[str, dict]]) -> None:
        if config_dict is None:
            return
        self.__rpc_listen_address = config_dict.get('external_trigger_rpc_address', self.__rpc_listen_address)

    def add_servicer_to_server(self, servicer, add_servicer_func: callable):
        self.__servicer_dict[servicer] = add_servicer_func

    async def startup(self):
        if self.__rpc_listen_address is None:
            return
