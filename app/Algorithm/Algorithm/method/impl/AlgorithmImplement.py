...
class AlgorithmImplement(AlgorithmInterface):
    ...
    async def run(self):
        try:
            ...
            channel_number = source_eeg_device.channel_number  # 获取数据源通道数
            ...
            while not data_finished_flag and not self._end_flag:
                ...
                self.__logger.info('yzl{}'.format(np.array(new_data).shape))
                if new_data is None:
                    continue
                ...
                if len(trial_start_index_list) != 0:
                    self.__logger.debug(f"收到trigger信息{trial_start_index_list}")
